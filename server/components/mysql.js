...
export const queryAsync = (connection, sql, values) =>
  new Promise((resolve, reject) => {
...
export async function upsert({ connection, table, record }) {
  ...
  const validColumns = (
    await query(`SELECT COLUMN_NAME FROM information_schema.COLUMNS where TABLE_NAME = ?`, table)
  ).map((e) => e.COLUMN_NAME);

  if (record.id) {
    // when upserting, columns without default values must be provided
    const [existingRecord] = await query(`SELECT * FROM ?? WHERE id = ?`, [table, record.id]);
  ...
  const valueAssignments = validColumns
    .map((column) => `${escape(column)} = COALESCE(VALUES(${escape(column)}), ${escape(column)}) `)
    .join(",");
