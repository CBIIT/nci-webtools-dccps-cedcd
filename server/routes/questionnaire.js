...
router.post("/select_owners_from_id", async function (req, res) {
  ...
  mysql.callProcedure(proc, params, function (result) {
    if (result && result[0][0]) res.json({ status: 200, data: result[0] });
...
router.post("/get_updated_cohortID", function (req, res) {
  ...
  mysql.callProcedure(proc, [req.body.oldID, req.body.newID], function (result) {
    if (result && result[0]) res.json({ status: 200, data: result[0][0].new_id });
    else res.json({ status: 400 });
  });
});
...
router.post("/upload/:id/:category", function (req, res, next) {
  ...
  fs.access(`${config.file_path}`, (err) => {
    ...
    if (Array.isArray(cohortFiles))
      cohortFiles.forEach((f) => {
        f.mv(`${config.file_path}/${f.name}`);
      });
    else cohortFiles.mv(`${config.file_path}/${cohortFiles.name}`);
  });
  res.json({ status: 200 });
...
router.post("/update_cohort_basic/:id", function (req, res) {
  ...
  for (let i in req.body.investigators) {
    req.body.investigators[i].name = req.body.investigators[i].name
      ? req.body.investigators[i].name.replace(/\n/g, "\\n")
      : "";
    req.body.investigators[i].institution = req.body.investigators[i].institution
      ? req.body.investigators[i].institution.replace(/\n/g, "\\n")
      : "";
    req.body.investigators[i].email = req.body.investigators[i].email
      ? req.body.investigators[i].email.replace(/\n/g, "\\n")
      : "";
  }
  ...
  mysql.callProcedure(func, params, function (results) {
      ...
      if (
        ...
        switch (k) {
          ...
          case "enrollment_year_end":
            if (!basic_info.enrollment_ongoing && !basic_info[k]) cohort_errors[k] = "Required field";
            break;
    ...
    if (
      ![
        ...
        "data_collected_web",
        "data_collected_other",
      ].some((k) => basic_info[k] === 1)
    )
      cohort_errors.dataCollection = "Required field";
    if (
      ![
        "requireNone",
        "requireCollab",
      ...
      cohort_errors.requirements = "Required field";
    if (
      ![
        "strategy_routine",
        "strategy_mailing",
      ...
      cohort_errors.strategy = "Required field";
    basic_info.cohort_description = basic_info.cohort_description
      ? basic_info.cohort_description.replace(/\\n/g, "\n")
      : "";
...
router.post("/upsert_enrollment_counts/:id", function (req, res) {
  ...
  mysql.callJsonProcedure(proc, params, function (result) {
    if (result && result[0] && result[0][0].rowsAffacted > 0) {
      ...
      } else res.json({ status: 200, message: "update successful" });
    } else res.json({ status: 500, message: "update failed" });
  });
});
...
router.post("/enrollment_counts/:id", function (req, res) {
  ...
  mysql.callProcedure(func, params, function (result) {
    ...
    for (let i = 0; i < result[0].length; i++) enrollmentCounts[result[0][i].cellId] = result[0][i].cellCount;
...
router.post("/mortality/:id", function (req, res) {
  ...
  mysql.callProcedure(func, params, function (result) {
    ...
    if (mortality) res.json({ status: 200, data: mortality });
    else res.json({ status: 500, message: "failed to load data" });
  });
});
...
router.post("/dlh/:id", function (req, res) {
  ...
  mysql.callProcedure(func, params, function (result) {
    ...
    logger.debug(dlh);
    if (Object.entries(dlh).length > 0) res.json({ status: 200, data: dlh });
    else res.json({ status: 500, message: "failed to load data" });
  });
});
...
router.get("/cancer_count/:id", function (req, res) {
  ...
  params.push(id);
  mysql.callProcedure(func, params, function (result) {
    if (result) res.json({ status: 200, data: result });
    else res.status(500).json({ status: 500, message: "failed to load data" });
  });
});
    ...
    if (result && result[0] && result[0].success === 1) {
      if (result[0].duplicated_cohort_id) {
        res.json({
          ...
          data: {
            duplicated_cohort_id: result[0].duplicated_cohort_id,
            status: result[0].status,
            sectionStatusList: result1,
          },
        });
      } else res.json({ status: 200, message: "update successful" });
    } else {
...
router.post("/update_specimen/:id", function (req, res) {
  ...
  keys.forEach((k) => (req.body[k] = req.body[k] ? req.body[k].replace(/\n/g, "\\n") : req.body[k]));
  let body = req.body;
  for (let k of Object.keys(body.counts)) {
    if (body.counts[k] === "") body.counts[k] = 0;
  }
  ...
  mysql.callProcedure(func, params, function (result) {
    if (result && result[0]) {
      ...
      if (result[3].length > 0) {
        for (let e of result[3]) if (e.email) temp.push(e.email);
        specimenData.emails = temp.join(",");
      }

      for (let k of result[0]) specimenData.counts[k.cellId] = k.counts;
      res.json({ status: 200, data: specimenData });
    } else res.json({ status: 500, message: "failed to retrieve data" });
  });
});
...
router.post("/reject/:id", function (request, response) {
  ...
  mysql.callProcedure(func, params1, function (result) {
    if (result && result[0] && result[0][0].rowAffacted > 0) response.json(true);
