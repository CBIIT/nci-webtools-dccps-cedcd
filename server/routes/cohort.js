...
router.post("/cancer", function (req, res) {
  ...
  mysql.callProcedure(func, params, function (results) {
    if (results && results[0] && results[0].length > 0) {
      ...
      races = races
        .map((x) =>
          x.race === "More Than One Race" || x.race === "Unknown or Not Reported" ? { ...x, race: "Z" + x.race } : x,
        )
        .sort((a, b) => a.race.localeCompare(b.race))
        .map((x) =>
          x.race === "ZMore Than One Race" || x.race === "ZUnknown or Not Reported"
            ? { ...x, race: x.race.slice(1) }
            : x,
        );

      allcancers.push({ id: 0, cancer: "All Cancer Types" });

      allcancers = allcancers
        .map((x) => (x.cancer === "All Other Cancers" ? { ...x, cancer: "Z" + x.cancer } : x))
      ...
      allcancers
        .filter(
          (e) =>
            filter.cancer.length === 0 ||
            filter.cancer.length === allcancers.length - 1 ||
            filter.cancer.includes(e.id),
        )
        .forEach(function (ac) {
          genders
            .filter((e) => filter.gender.length !== 1 || filter.gender.includes(e.id))
            .forEach(function (ge) {
              ethnicities
                .filter((e) => filter.ethnicity.length === 0 || filter.ethnicity.includes(e.id))
                .forEach(function (et) {
                  races
                    .filter((e) => filter.race.length === 0 || filter.race.includes(e.id))
                    .forEach(function (ra) {
    ...
    if (results && results[0] && results[0].length > 0) {
      ...
      if ([0, 1].includes(basic.request_procedures_none)) {
        if (basic.request_procedures_none === 1) {
          info.request_procedures_web_url = results[1].find(
            (f) => f.attachment_type === 0 && f.category === 5 && f.status === 1,
          )
            ? results[1].find((f) => f.attachment_type === 0 && f.category === 5 && f.status === 1).website
            : "";
        } else {
          results[1].forEach((f) => {
