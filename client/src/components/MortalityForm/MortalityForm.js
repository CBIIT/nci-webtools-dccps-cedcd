...
const MortalityForm = ({ ...props }) => {
  ...
  const validateInput = () => {
    ...
    else if (+mortality.mortalityYear > new Date().getFullYear()) copy.mortalityYear = "No future year is allowed";
    ...
    if (!mortality.deathIndex && !mortality.deathCertificate && !mortality.otherDeath) {
      ...
      if (mortality.otherDeath === 1) {
        if (mortality.otherDeathSpecify === null || !mortality.otherDeathSpecify)
          ...
          if (mortality.otherDeathSpecify.length > 200) copy.otherDeathSpecify = "Cannot exceed 200 characters";
          else copy.otherDeathSpecify = "";
        }
      } else copy.otherDeathSpecify = "";
    }

    if (!(mortality.haveDeathDate in [0, 1])) {
      copy.haveDeathDate = radioError;
    } else {
      copy.haveDeathDate = "";
    }
    if (!(mortality.haveDeathCause in [0, 1])) {
      copy.haveDeathCause = radioError;
    } else {
      copy.haveDeathCause = "";
    }
    if (mortality.haveDeathCause === 1) {
      ...
      if (mortality.otherCode === 1) {
        if (mortality.otherCodeSpecify === null || !mortality.otherCodeSpecify)
          ...
          if (mortality.otherCodeSpecify.length > 200) copy.otherCodeSpecify = "Cannot exceed 200 characters";
          else copy.otherCodeSpecify = "";
        }
      } else copy.otherCodeSpecify = "";
    } else {
      copy.coded = "";
    ...
    if (mortality.deathNumbers === 0 || mortality.deathNumbers === "0") copy.deathNumbers = "";
    else copy.deathNumbers = validator.numberValidator(mortality.deathNumbers, true, false);
          ...
          <CollapsiblePanel condition={activePanels.A} onClick={(_) => toggleActivePanel("A")} panelTitle="Mortality">
              ...
              <Col sm="2">
                <Reminder message={errors.mortalityYear} disabled={!(saved && errors.mortalityYear)} placement="right">
                  <Form.Control
                    ...
                    readOnly={isReadOnly}
                    onChange={(e) => {
                      dispatch(allactions.mortalityActions.setMortalityYear(e.target.value));
            ...
            <Form.Group as={Row} className={saved && errors.otherDeathSpecify && "has-error"}>
              ...
              <Col sm="12">
                  ...
                  <Form.Check className="pl-0" name="deathCertificate">
                    <Form.Check.Input
                      ...
                      onClick={() => {
                        if (!isReadOnly) {
                          dispatch(
                            allactions.mortalityActions.setDeathCertificate((mortality.deathCertificate + 1) % 2),
                          );
                          dispatch(setHasUnsavedChanges(true));
                        }
                      }}
                ...
                <Reminder
                  message={errors.otherDeathSpecify}
                  disabled={!(saved && errors.otherDeathSpecify)}
                  placement="right">
                  <Form.Control
                    type="text"
            ...
            <Form.Group as={Row} className={saved && errors.otherCodeSpecify && "has-error"}>
              ...
              <Col sm="12">
                ...
                <Reminder
                  message={errors.otherCodeSpecify}
                  disabled={!(saved && errors.otherCodeSpecify)}
                  placement="right">
                  <Form.Control
                    type="text"
