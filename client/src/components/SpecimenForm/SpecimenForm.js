...
const SpecimenForm = ({ ...props }) => {
  ...
  const [g1to6FlagList, setG1to6FlagList] = useState({
    bioBloodBaseline: 1,
    bioBloodOtherTime: 1,
    bioBuccalSalivaBaseline: 1,
    bioBuccalSalivaOtherTime: 1,
    bioTissueBaseline: 1,
    bioTissueOtherTime: 1,
    bioUrineBaseline: 1,
    bioUrineOtherTime: 1,
    bioFecesBaseline: 1,
    bioFecesOtherTime: 1,
  ...
  const refreshErrors = () =>
    (errors.bioBloodBaseline && errors.bioBloodOtherTime) /* G1 */ ||
    (+specimen.bioBloodBaseline === 1 &&
      specimen.bioBloodBaselineSerum === 0 &&
      specimen.bioBloodBaselinePlasma === 0 &&
      specimen.bioBloodBaselineBuffyCoat === 0 &&
      specimen.bioBloodBaselineOtherDerivative === 0) ||
    (specimen.bioBloodOtherTime === 1 &&
      specimen.bioBloodOtherTimeSerum === 0 &&
      specimen.bioBloodOtherTimePlasma === 0 &&
      specimen.bioBloodOtherTimeBuffyCoat === 0 &&
      specimen.bioBloodOtherTimeOtherDerivative === 0) ||
    ...
    (+specimen.bioMetabolomicData === 1 &&
      ...
      errors.bioMetaOutcomesInCvdStudy &&
      errors.bioMetaOutcomesInDiabetesStudy &&
      errors.bioMetaOutcomesInOtherStudy) ||
      ...
      .then((result) => {
        ...
        if (result && specimenCounts) {
          batch(() => {
            ...
            for (let k of Object.keys(specimenInfo)) {
              ...
              switch (specimenInfo[k].sub_category) {
                ...
                case "bio_meta_outcomes_in_cvd_study": // specimen_id 43
                  ...
                  if (specimenInfo[k].collected_yn)
                    dispatch(allactions.specimenErrorActions.bioMetaOutcomesInCvdStudy(k_field_status));
                  break;
                case "bio_meta_outcomes_in_diabetes_study": // specimen_id 44
                  ...
                  if (specimenInfo[k].collected_yn)
                    dispatch(allactions.specimenErrorActions.bioMetaOutcomesInDiabetesStudy(k_field_status));
                  break;
            ...
            dispatch(
              allactions.specimenActions.bioLabsUsedForAnalysis(specimenDetails.bio_labs_used_for_analysis || ""),
            );
            dispatch(allactions.specimenActions.bioMemberInStudy(specimenDetails.bio_member_in_study || ""));
            dispatch(
              allactions.specimenActions.bioNumberMetabolitesMeasured(
                specimenDetails.bio_number_metabolites_measured || "",
              ),
            );
            dispatch(
              allactions.specimenActions.bioOtherBaselineSpecify(specimenDetails.bio_other_baseline_specify || ""),
            );
            dispatch(
              allactions.specimenActions.bioOtherOtherTimeSpecify(specimenDetails.bio_other_other_time_specify || ""),
            );
            ...
            dispatch(
              allactions.specimenErrorActions.bioAnalyticalPlatform(!isNull(specimenDetails.bio_analytical_platform)),
            );
            dispatch(
              allactions.specimenErrorActions.bioLabsUsedForAnalysis(
                !isNull(specimenDetails.bio_labs_used_for_analysis),
              ),
            );
            dispatch(allactions.specimenErrorActions.bioMemberInStudy(!isNull(specimenDetails.bio_member_in_study)));
            dispatch(
              allactions.specimenErrorActions.bioMetaOutcomesOtherStudySpecify(
                !isNull(specimenDetails.bio_meta_outcomes_other_study_specify),
              ),
            );
            dispatch(
              allactions.specimenErrorActions.bioNumberMetabolitesMeasured(
                !isNull(specimenDetails.bio_number_metabolites_measured),
              ),
            );
            dispatch(
              allactions.specimenErrorActions.bioOtherBaselineSpecify(
                !isNull(specimenDetails.bio_other_baseline_specify),
              ),
            );
            dispatch(
              allactions.specimenErrorActions.bioOtherOtherTimeSpecify(
                !isNull(specimenDetails.bio_other_other_time_specify),
              ),
            );
            dispatch(
              allactions.specimenErrorActions.bioSeparationPlatform(!isNull(specimenDetails.bio_separation_platform)),
            );
            if (
              specimenDetails.bio_year_samples_sent &&
              +specimenDetails.bio_year_samples_sent > 1900 &&
              +specimenDetails.bio_year_samples_sent < 2100
            ) {
              dispatch(allactions.specimenErrorActions.bioYearSamplesSent(true));
            } else {
              dispatch(allactions.specimenErrorActions.bioYearSamplesSent(false));
    ...
    fetch(`/api/questionnaire/update_specimen/${id}`, {
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          if (!errorsRemain) dispatch(allactions.sectionActions.setSectionStatus("G", "complete"));
          else {
  ...
  useEffect(() => {
    let tempStatus =
      specimen.bioBloodBaseline === 0 &&
      specimen.bioBloodOtherTime === 0 &&
      specimen.bioBuccalSalivaBaseline === 0 &&
      specimen.bioBuccalSalivaOtherTime === 0 &&
      specimen.bioTissueBaseline === 0 &&
      specimen.bioTissueOtherTime === 0 &&
      specimen.bioUrineBaseline === 0 &&
      specimen.bioUrineOtherTime === 0 &&
      specimen.bioFecesBaseline === 0 &&
      specimen.bioFecesOtherTime === 0 &&
      specimen.bioOtherBaseline === 0 &&
      specimen.bioOtherOtherTime === 0;
  ...
  function RadioButtonInput({ field_id, disabled_id }) {
    ...
    return options.map(({ label, value }, i) => (
      <Form.Check
        ...
        label={label}
        disabled={
          g1to6List.includes(key) ? "" : g1to6Flag ? true : isNull(disabled_id) ? "" : +specimen[disabled_id] !== 1
        }
        // disabled={!g1to6Flag}
        ...
        onChange={(e) => {
          if (!isReadOnly) {
            ...
            else if (+value === 0 && key === "bioOtherBaseline")
              dispatch(allactions.specimenActions.bioOtherBaselineSpecify(""));
            else if (+value === 0 && key === "bioOtherOtherTime")
  ...
  function CheckBoxInput({ field_id, label, disabled_id }) {
    ...
    return (
      <Form.Check type="checkbox" className="pl-0" id={key} key={key} name={key}>
        <Form.Check.Input
          bsPrefix
          type="checkbox"
          className="mr-2"
  ...
  function CheckBoxInputs({ optionList }) {
    return optionList.map((option, i) => <CheckBoxInput key={i} {...option} />);
  }
              ...
              <Col className="mb-0 pl-0" sm="12">
                <Col sm="5">
                  <label className="required-label">
                    {item[v].field_id.includes("Baseline") ? title[0] : title[1]}
                  </label>
                </Col>
                <Col sm="7">
  ...
  const getPartContent = (partSelect = "A") => {
    return fieldList.specimenFieldList
      .filter((field) => field.part === partSelect)
      .map((field) => {
        if (field.title !== "G.6 Other(e.g. toenails)") {
            ...
            <Form.Group as={Row} sm="12" className="mb-0">
              ...
              <Col
                ...
                className={classNames(
                  "form-group",
                  "align-self-center",
                  saved && +specimen.bioOtherBaseline === 1 && errors.bioOtherBaselineSpecify && "has-error",
                )}>
                <Form.Label className="pl-0" column sm="12" style={{ fontWeight: "normal" }}>
                  If yes, please specify
                </Form.Label>
                <Reminder
                  message={"Required Field"}
                  disabled={!(saved && +specimen.bioOtherBaseline === 1 && errors.bioOtherBaselineSpecify)}
                  addspan={true}
                  placement="right">
                  <Form.Control
                    type="text"
              ...
              <Col sm="12" className="mb-0 pl-0">
                ...
                <Col sm="3" className="align-self-center">
                  ...
                  {errors.bioOtherOtherTime && saved && (
                    <span className="text-danger ml-3 font-weight-normal">Required Field</span>
                  )}
                </Col>
              </Col>
              ...
              <Col
                ...
                className={classNames(
                  "form-group",
                  "align-self-center",
                  saved && errors.bioOtherOtherTimeSpecify && specimen.bioOtherOtherTime === 1 && "has-error",
                )}>
                <Form.Label className="pl-0" column sm="12" style={{ fontWeight: "normal" }}>
                  If yes, please specify
                </Form.Label>
                <Reminder
                  message={"Required Field"}
                  disabled={!(saved && +specimen.bioOtherOtherTime === 1 && errors.bioOtherOtherTimeSpecify)}
                  addspan={true}
                  placement="right">
                  <Form.Control
                    type="text"
  ...
  return (
    <Container>
        ...
        footer={
          <>
            <Button
              ...
              variant="secondary"
              onClick={(_) => updateRejectionModal({ show: false })}>
              Cancel
            </Button>
            <Button
              className="col-lg-2 col-md-6"
              variant="primary"
              disabled={!rejectionModal.notes}
      ...
      <ReviewApproveModal
        show={reviewApproveModalShow}
        title={<span>Publish Cohort</span>}
        body={
          <span>
            This submitted cohort will be published on the CEDCD public website once you click on Publish. Are you sure
            you want to continue?
          </span>
        }
        footer={
          <div>
            <Button variant="secondary" className="col-lg-2 col-md-6" onClick={(_) => setReviewApproveModalShow(false)}>
              Cancel
            </Button>
            <Button variant="primary" className="col-lg-2 col-md-6" onClick={(_) => handleApprove()}>
              Publish
            </Button>
          </div>
        }
      ...
      <Form>
        <CollapsiblePanelContainer>
          ...
          <CollapsiblePanel
            ...
            <div>
              <p>
                Specify the types of specimens you collected, whether the specimen was collected at baseline, and/or
            ...
            <Form.Group as={Row}>
                ...
                <Col sm="12">
                  ...
                  <CheckBoxInputs
                    optionList={[
                      ...
                      {
                        field_id: "bioBloodBaselineOtherDerivative",
                        label: "Other Blood Derivative",
                        disabled_id: "bioBloodBaseline",
                      },
                    ]}
                  />
                </Col>
              ...
              <Col className="mb-0 pl-0" sm="12">
                ...
                <Col sm="3" className="align-self-center">
                  ...
                  {errors.bioBloodOtherTime && saved && (
                    <span className="text-danger ml-3 font-weight-normal">Required Field</span>
                  )}
                </Col>
                <Col sm="12">
                  ...
                  <CheckBoxInputs
                    optionList={[
                      ...
                      {
                        field_id: "bioBloodOtherTimeOtherDerivative",
                        label: "Other Blood Derivative",
                        disabled_id: "bioBloodOtherTime",
                      },
                    ]}
                  />
                </Col>
          ...
          <CollapsiblePanel
            ...
            <div>
              <p>Do you collect the following data?</p>
            </div>
            {getPartContent("B")}
          </CollapsiblePanel>
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              <Form.Label column sm="12">
                ...
                {+specimen.bioMetabolomicData === 1 &&
                  ...
                  errors.bioMetaOutcomesInOtherStudy &&
                  saved &&
                  !g1to6Flag && <span className="text-danger ml-3 font-weight-normal">Required Field</span>}
              </Form.Label>
                ...
                <Col sm="12">
                  <CheckBoxInputs
                    optionList={[
                      ...
                      {
                        field_id: "bioMetaOutcomesInDiabetesStudy",
                        label: "Diabetes",
                        disabled_id: "bioMetabolomicData",
                      },
                      {
                        field_id: "bioMetaOutcomesInOtherStudy",
                        label: "Other, specify:",
                        disabled_id: "bioMetabolomicData",
                      },
                    ]}
              ...
              <Col sm="12" className="align-self-center">
                <Reminder
                  ...
                  disabled={
                    !(
                      +specimen.bioMetaOutcomesInOtherStudy === 1 &&
                      +specimen.bioMetabolomicData === 1 &&
                      errors.bioMetaOutcomesOtherStudySpecify &&
                      saved
                    )
                  }
                  addspan={true}>
                  <Form.Control
                    ...
                    onBlur={(e) =>
                      dispatch(
                        allactions.specimenErrorActions.bioMetaOutcomesOtherStudySpecify(!isNull(e.target.value)),
                      )
                    }
                  />{" "}
                </Reminder>
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="2">
                <Reminder
                  message="Required Field"
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioMemberInStudy && saved)}
                  addspan={true}>
                  <Form.Control
                    type="text"
                    className="resize-vertical"
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <Reminder
                  message="Required Field"
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioLabsUsedForAnalysis && saved)}
                  addspan={true}>
                  <Form.Control
                    as="textarea"
                    className="resize-vertical"
                    ...
                    onBlur={(e) =>
                      dispatch(allactions.specimenErrorActions.bioLabsUsedForAnalysis(!isNull(e.target.value)))
                    }
                  />{" "}
                </Reminder>
              </Col>
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <Reminder
                  message="Required Field"
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioAnalyticalPlatform && saved)}
                  addspan={true}>
                  <Form.Control
                    as="textarea"
                    className="resize-vertical"
                    ...
                    onBlur={(e) =>
                      dispatch(allactions.specimenErrorActions.bioAnalyticalPlatform(!isNull(e.target.value)))
                    }
                  />{" "}
                </Reminder>
              </Col>
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <Reminder
                  message="Required Field"
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioSeparationPlatform && saved)}
                  addspan={true}>
                  <Form.Control
                    as="textarea"
                    className="resize-vertical"
                    ...
                    onBlur={(e) =>
                      dispatch(allactions.specimenErrorActions.bioSeparationPlatform(!isNull(e.target.value)))
                    }
                  />{" "}
                </Reminder>
              </Col>
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="2">
                <Reminder
                  message="Required Field"
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioNumberMetabolitesMeasured && saved)}
                  addspan={true}>
                  <Form.Control
                    type="text"
                    style={
                      (+specimen.bioMetabolomicData === 1 &&
                        errors.bioNumberMetabolitesMeasured &&
                        saved && { border: "1px solid red" }) ||
                      {}
                    }
                    ...
                    onBlur={(e) =>
                      dispatch(allactions.specimenErrorActions.bioNumberMetabolitesMeasured(!isNull(e.target.value)))
                    }
                  />{" "}
                </Reminder>
              </Col>
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="2">
                <Reminder
                  message={g15yearErrMsg || "Required Field"}
                  disabled={!(+specimen.bioMetabolomicData === 1 && errors.bioYearSamplesSent && saved)}
                  addspan={true}>
                  <Form.Control
                    type="text"
                    style={
          ...
          <CollapsiblePanel
            ...
            <div className="my-3">
              ...
              <div>
                {" "}
                Please complete this table with the number of individuals with biospecimens available in your current
                inventory. If you do not have exact counts, please enter approximate counts. (Note, please record the
                number of individual participants for whom there are available samples– NOT the number of aliquots.)
              </div>
            </div>
