...
const CohortForm = ({ ...props }) => {
        ...
        if (result.status === 200) {
          ...
          if (result.newCohortInfo.newCohortID && result.newCohortInfo.newCohortID != cohortID) {
            ...
            if (secStatusList && secStatusList.length > 0)
              secStatusList.forEach((item, idx) => {
            ...
            window.history.pushState(
              ...
              window.location.pathname.replace(/\d+$/, result.newCohortInfo.newCohortID),
            );
          } else {
            dispatch(fetchCohort(cohortID));
          }
          if (result.newCohortInfo.investigators)
            dispatch(allactions.cohortActions.setInvestigators(result.newCohortInfo.investigators));
  ...
  const getMinAgeValidationResult = (value, requiredOrNot, maxAge, medianAge, meanAge) =>
    validator.minAgeValidator(value, requiredOrNot, maxAge, medianAge, meanAge);
  const getMaxAgeValidationResult = (value, requiredOrNot, minAge, medianAge, meanAge) =>
    validator.maxAgeValidator(value, requiredOrNot, minAge, medianAge, meanAge);
  const getMeanMedianAgeValidationResult = (value, requiredOrNot, minAge, maxAge) =>
    validator.medianAgeValidator(value, requiredOrNot, minAge, maxAge);
  ...
  const populateErrors = (fieldName, value, requiredOrNot, valueType) => {
    ...
    if (!result) {
      if (valueType === "startyear" && value && cohort.enrollment_year_end > 0 && value > cohort.enrollment_year_end)
        ...
        else dispatch(allactions.cohortErrorActions.enrollment_year_end(false, "expecting a year value in the past"));
      }
      if (valueType === "endyear" && value && cohort.enrollment_year_start > 0 && value < cohort.enrollment_year_start)
        ...
        else dispatch(allactions.cohortErrorActions.enrollment_year_start(false, "expecting a year value in the past"));
      }
    }
  ...
  const updateErrors = (
    ...
    if (currentState) {
      //if any of the checkboxes is checked remove error
      dispatch(allactions.cohortErrorActions[errorname](true));
    } else {
      // if none of them is checked
          ...
          <div className="mb-3">
            {files
              ...
              .map((f) => (
                <div className="my-1">
                  <Col md="10">
                    <a href={"../../../api/download/" + f.filename} download target="_blank">
                      {f.filename}
                    </a>
                  </Col>
                  <Col md="2" className="text-center">
                    <span>
                      {!isReadOnly && (
                        <span
                          className="closer"
                          onClick={() => deleteFileFromList(fileListName, f.filename, f.fileId, cohortID)}>
                          x
                        </span>
                      )}
                    </span>
                  ...
                  <Col md="10" className="col-9">
                    <Reminder message={url} info={true} disabled={url.length <= maxLength}>
                      <a href={url} target="_blank">
                        {url.length > maxLength ? url.substr(0, maxLength) + "..." : url}
                      </a>
                    </Reminder>
                  </Col>
  ...
  return (
    <Container>
      ...
      <FileModal
        ...
        footer={
          <div className="d-flex">
            <Button variant="secondary" onClick={() => setUrlModal(false)} className="col-lg-2 col-md-6">
              Close
            </Button>
            <Button
              variant="primary"
      ...
      <FileModal
        ...
        footer={
          <Button variant="primary" onClick={() => setFileModal(false)} className="col-lg-2 col-md-6">
            Close
          </Button>
        }
      />
      ...
      <FileModal
        show={urlList}
        handleClose={() => setUrlListModal(false)}
        body={url_list(urlTitle, urlTile, currentUrlList)}
        footer={
          <Button variant="primary" onClick={() => setUrlListModal(false)} className="col-lg-2 col-md-6">
            Close
          </Button>
        }
      />
      ...
      <div style={{ marginTop: "20px", marginBottom: "20px" }}>
        If your cohort is comprised of more than one distinct enrollment period or population, please complete separate
        CEDCD Data Collection Forms to treat them as separate cohorts
      </div>
      <Form>
        <CollapsiblePanelContainer>
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <p>
                  Please provide a short paragraph describing your cohort. This will be used as an overall narrative
                  description of your cohort on the CEDCD website. You may provide a link to a description on your
                  cohort’s website.
                </p>
              </Col>
              <Col sm="12">
                <Form.Control
                  ...
                  onChange={(e) => dispatch(allactions.cohortActions.cohort_description(e.target.value))}
                  readOnly={isReadOnly}
                />
              </Col>
            </Form.Group>
                ...
                {errors.cohort_web_site && saved ? (
                  <Reminder message={errors.cohort_web_site}>
                    <Form.Control
                      ...
                      onChange={(e) => dispatch(allactions.cohortActions.cohort_web_site(e.target.value))}
                      onBlur={(e) => populateErrors("cohort_web_site", e.target.value, false, "string")}
                      readOnly={isReadOnly}
                    />
                  </Reminder>
                ) : (
                        ...
                        <Form.Check.Input
                          ...
                          onClick={(e) => {
                            ...
                            if (!isReadOnly) {
                              ...
                              dispatch(allactions.cohortErrorActions.clarification_contact(true));
                              !cohort.contacterName &&
                                dispatch(allactions.cohortErrorActions.contacterName(false, "Required Field"));
                              !cohort.contacterPosition &&
                                dispatch(allactions.cohortErrorActions.contacterPosition(false, "Required Field"));
                              if (cohort.contacterPhone && phoneCheckResult)
                                dispatch(allactions.cohortErrorActions.contacterPhone(false, phoneCheckResult));
                              if (!cohort.contacterEmail)
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              <Form.Label column sm="12">
                A.6 If an investigator is interested in collaborating with your cohort on a new project, whom should
                they contact?
              </Form.Label>
              <Col sm="6">
                <Person
              ...
              <Col sm="6">
                <div key="checkbox">
                  <Form.Check type="checkbox" className="pl-0" id="default-completerName-check" name="sameAsCompleted">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                      ...
                      onClick={(e) => {
                        ...
                        if (!isReadOnly) {
                          ...
                          if (e.target.checked) {
                            ...
                            if (cohort.collaboratorPhone && phoneCheckResult)
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-contacterName-check" name="sameAsContacted">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12" className="p-0 mb-3">
                ...
                <Col sm="12" className="mb-3">
                  <div key="checkbox">
                    <Form.Check type="checkbox" className="pl-0" id="default-cancerSurvivors" name="cancerSurvivors">
                      <Form.Check.Input
                        bsPrefix
                        type="checkbox"
                        className="mr-2"
                ...
                <Col sm="12">
                  <Form.Control
                    ...
                    onChange={(e) => dispatch(allactions.cohortActions.eligible_disease_cancer_specify(e.target.value))}
                  />
                </Col>
              </Col>
            ...
            <Form.Group as={Row}>
                  ...
                  {errors.enrollment_total && saved ? (
                    <Reminder message={errors.enrollment_total}>
                      <Form.Control
                        ...
                        onBlur={(e) => populateErrors("enrollment_total", e.target.value, true, "number")}
                      />
                    </Reminder>
                  ) : (
                    <Form.Control
                  ...
                  {errors.enrollment_year_start && saved ? (
                    <Reminder message={errors.enrollment_year_start}>
                      <Form.Control
                        ...
                        onBlur={(e) => populateErrors("enrollment_year_start", e.target.value, true, "startyear")}
                      />
                    </Reminder>
                  ) : (
                    <Form.Control
                        ...
                        <Form.Check.Input
                          ...
                          onClick={() => {
                            ...
                            if (!isReadOnly) {
                              ...
                              cohort.enrollment_year_complete &&
                        ...
                        <Form.Check.Input
                          ...
                          onClick={() => {
                            if (!isReadOnly) {
                              ...
                              dispatch(allactions.cohortErrorActions.enrollment_year_end(true));
                              if (!cohort.enrollment_target)
                                dispatch(allactions.cohortErrorActions.enrollment_target(false, "Required Field"));
                              else if (cohort.enrollment_target < 0)
                                dispatch(allactions.cohortErrorActions.enrollment_target(false, "Invalid value"));
                              if (!cohort.enrollment_year_complete)
                                dispatch(
                                  allactions.cohortErrorActions.enrollment_year_complete(false, "Requred Filed"),
                                );
                              else if (cohort.enrollment_year_complete < 999)
                                dispatch(
                                  allactions.cohortErrorActions.enrollment_year_complete(false, "Invalid value"),
                                );
                            }
                          }}
                        />
                ...
                <Col sm="2">
                  <Reminder message={errors.enrollment_year_end} disabled={!(errors.enrollment_year_end && saved)}>
                    <input
                      ...
                      onBlur={(e) => populateErrors("enrollment_year_end", e.target.value, true, "endyear")}
              ...
              <Col sm="12" className="p-0 mb-1">
                ...
                <Col sm="2">
                  <Reminder
                    message={errors.enrollment_year_complete}
                    disabled={!(errors.enrollment_year_complete && saved)}>
                    <Form.Control
                      type="text"
                      style={
                  ...
                  <InputGroup>
                    <Reminder message={errors.enrollment_age_min} disabled={!(errors.enrollment_age_min && saved)}>
                      <Form.Control
                        ...
                        onChange={(e) =>
                          !isReadOnly &&
                          /^\s*\d*\s*$/.test(e.target.value) &&
                          dispatch(allactions.cohortActions.enrollment_age_min(e.target.value))
                        }
                        onBlur={(e) => {
                          if (!isReadOnly) {
                            if (!e.target.value)
                              dispatch(allactions.cohortErrorActions.enrollment_age_min(false, "Required Filed"));
                    ...
                    <Reminder message={errors.enrollment_age_max} disabled={!(errors.enrollment_age_max && saved)}>
                      <Form.Control
                        ...
                        onChange={(e) =>
                          !isReadOnly &&
                          /^\s*\d*\s*$/.test(e.target.value) &&
                          dispatch(allactions.cohortActions.enrollment_age_max(e.target.value))
                        }
                        onBlur={(e) => {
                          if (!isReadOnly) {
                            if (!e.target.value)
                              dispatch(allactions.cohortErrorActions.enrollment_age_max(false, "Required Filed"));
                  ...
                  <Reminder message={errors.enrollment_age_median} disabled={!(errors.enrollment_age_median && saved)}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        !isReadOnly &&
                        /^\s*\d*\s*$/.test(e.target.value) &&
                        dispatch(allactions.cohortActions.enrollment_age_median(e.target.value))
                      }
                      onBlur={(e) => {
                        if (!isReadOnly) {
                          if (!e.target.value)
                            dispatch(allactions.cohortErrorActions.enrollment_age_median(false, "Required Filed"));
                  ...
                  <Reminder message={errors.enrollment_age_mean} disabled={!(errors.enrollment_age_mean && saved)}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        !isReadOnly &&
                        /^\s*\d*\s*$/.test(e.target.value) &&
                        dispatch(allactions.cohortActions.enrollment_age_mean(e.target.value))
                      }
                      onBlur={(e) => {
                        if (!e.target.value)
                          dispatch(allactions.cohortErrorActions.enrollment_age_mean(false, "Required Filed"));
                    ...
                    <Reminder message={errors.current_age_min} disabled={!(errors.current_age_min && saved)}>
                      <Form.Control
                        ...
                        onChange={(e) =>
                          !isReadOnly &&
                          /^\s*\d*\s*$/.test(e.target.value) &&
                          dispatch(allactions.cohortActions.current_age_min(e.target.value))
                        }
                        onBlur={(e) => {
                          if (!e.target.value)
                            dispatch(allactions.cohortErrorActions.current_age_min(false, "Required Filed"));
                    ...
                    <Reminder message={errors.current_age_max} disabled={!(errors.current_age_max && saved)}>
                      <Form.Control
                        ...
                        onChange={(e) =>
                          !isReadOnly &&
                          /^\s*\d*\s*$/.test(e.target.value) &&
                          dispatch(allactions.cohortActions.current_age_max(e.target.value))
                        }
                        onBlur={(e) => {
                          if (!e.target.value)
                            dispatch(allactions.cohortErrorActions.current_age_max(false, "Required Filed"));
                  ...
                  <Reminder message={errors.current_age_median} disabled={!(errors.current_age_median && saved)}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        !isReadOnly &&
                        /^\s*\d*\s*$/.test(e.target.value) &&
                        dispatch(allactions.cohortActions.current_age_median(e.target.value))
                      }
                      onBlur={(e) => {
                        if (!e.target.value)
                          dispatch(allactions.cohortErrorActions.current_age_median(false, "Required Filed"));
                  ...
                  <Reminder message={errors.current_age_mean} disabled={!(errors.current_age_mean && saved)}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        !isReadOnly &&
                        /^\s*\d*\s*$/.test(e.target.value) &&
                        dispatch(allactions.cohortActions.current_age_mean(e.target.value))
                      }
                      onBlur={(e) => {
                        if (!e.target.value)
                          dispatch(allactions.cohortErrorActions.current_age_mean(false, "Required Filed"));
          ...
          <CollapsiblePanel
                ...
                {errors.time_interval && saved ? (
                  <Reminder message={errors.time_interval}>
                    <Form.Control
                      ...
                      onChange={(e) => !isReadOnly && dispatch(allactions.cohortActions.time_interval(e.target.value))}
                      onBlur={(e) => populateErrors("time_interval", e.target.value, true, "string")}
                    />
                  </Reminder>
                ) : (
                  <Form.Control
                  ...
                  <Reminder message={errors.most_recent_year}>
                    <Form.Control
                      ...
                      onChange={(e) =>
                        !isReadOnly &&
                        e.target.value.length <= 4 &&
                        dispatch(allactions.cohortActions.most_recent_year(e.target.value))
                      }
                      placeholder="YYYY"
                      onBlur={(e) => populateErrors("most_recent_year", e.target.value, true, "most_recent_year")}
                    />
                  </Reminder>
                ) : (
                  <Form.Control
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <div key="checkbox" className="mb-3">
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-collected-other" name="data_collected_other">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                ...
                {saved && errors.data_collected_other_specify ? (
                  <Reminder message={errors.data_collected_other_specify}>
                    <Form.Control
                      ...
                      onBlur={() =>
                        cohort.data_collected_other &&
                        populateErrors(
                          "data_collected_other_specify",
                          cohort.data_collected_other_specify,
                          true,
                          "string",
                        )
                      }
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <div key="checkbox" className="mb-3">
                  <Form.Check type="checkbox" className="pl-0" id="default-require-none" name="requireNone">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-require-irb" name="requireIrb">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-restrict-geno-info" name="restrictGenoInfo">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-restrict-other-db" name="restrictOtherDb">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <div key="checkbox" className="mb-3">
                  <Form.Check type="checkbox" className="pl-0" id="default-strategy-routine" name="strategy_routine">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-strategy-mailing" name="strategy_mailing">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
                    ...
                    />
                    <Form.Check.Label style={{ fontWeight: "normal" }}>
                      Send newsletters or other information or personal mailings unrelated to data collection (e.g.,
                      birthday cards)
                    </Form.Check.Label>
                  </Form.Check>
                  <Form.Check
                  ...
                  <Form.Check type="checkbox" className="pl-0" id="default-strategy-other" name="strategy_other">
                    <Form.Check.Input
                      bsPrefix
                      type="checkbox"
                      className="mr-2"
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                <p>
                  As indicated on the CEDCD Approval Form, we are requesting the following items for inclusion on the
                  CEDCD website. If you provided approval to post this information, please attach the documents and
                  return them with this form. If they are already available on a publicly accessible website, please
                  just provide the website address.
                </p>
              </Col>
              <Col sm="12">
                ...
                <div className="table-responsive d-md-none">
                  <Table bordered condensed className="table-valign-middle">
                    <tbody>
                      <tr>
                        <th className="align-middle" style={{ backgroundColor: "#01857b", color: "white" }}>
                          Questionnaire
                        </th>
                        <td className="p-0">
                          <Table
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col
                                      ...
                                      md={!isReadOnly ? "12" : "11"}
                                      lg={!isReadOnly ? "7" : "11"}>
                                      {cohort.questionnaire_url.length === 0 && <span>No URL(s) entered</span>}
                                      {cohort.questionnaire_url.length > 0 && (
                                        <span>
                                          <a href={cohort.questionnaire_url[0]} target="_blank">
                              ...
                              <tr>
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col sm="12">
                                      {QfileLoading && <span>Loading...</span>}
                                      {!QfileLoading && cohort.questionnaireFileName.length === 0 && (
                                        <span>{isReadOnly ? "No file(s) uploaded" : "No file chosen"}</span>
                                      )}
                                      {cohort.questionnaireFileName.length > 0 && (
                                        <span>
                                          <a
                                            href={"../../../api/download/" + cohort.questionnaireFileName[0].filename}
                                            download={
                                              cohort.questionnaireFileName[0].filename.split(".").pop() === "pdf"
                                                ? false
                                                : true
                                            }
                                            target="_blank">
                                              ...
                                              <span
                                                ...
                                                onClick={() =>
                                                  deleteFileFromList(
                                                    ...
                                                    cohort.questionnaireFileName[0].filename,
                                                    cohort.questionnaireFileName[0].fileId,
                                                    cohortID,
                                                  )
                                                }>
                                                x
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col
                                      ...
                                      md={!isReadOnly ? "12" : "11"}
                                      lg={!isReadOnly ? "7" : "11"}>
                                      {cohort.main_cohort_url.length === 0 && <span>No URL(s) entered</span>}
                                      {cohort.main_cohort_url.length > 0 && (
                                        <span>
                                          <a href={cohort.main_cohort_url[0]} target="_blank">
                              ...
                              <tr>
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col sm="12">
                                      {MfileLoading && <span>Loading...</span>}
                                      ...
                                      {cohort.mainFileName.length > 0 && (
                                        <span>
                                          <a
                                            href={"../../../api/download/" + cohort.mainFileName[0].filename}
                                            download={
                                              cohort.mainFileName[0].filename.split(".").pop() === "pdf" ? false : true
                                            }
                                            target="_blank">
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col
                                      ...
                                      md={!isReadOnly ? "12" : "11"}
                                      lg={!isReadOnly ? "7" : "11"}>
                                      {cohort.publication_url.length === 0 && <span>No URL(s) entered</span>}
                                      {cohort.publication_url.length > 0 && (
                                        <span>
                                          <a href={cohort.publication_url[0]} target="_blank">
                              ...
                              <tr>
                                ...
                                <td>
                                  <Row className="w-100">
                                    ...
                                    <Col sm="12">
                                      {PfileLoading && <span>Loading...</span>}
                                      ...
                                      {cohort.publicationFileName.length > 0 && (
                                        <span>
                                          <a
                                            href={"../../../api/download/" + cohort.publicationFileName[0].filename}
                                            download={
                                              cohort.publicationFileName[0].filename.split(".").pop() === "pdf"
                                                ? false
                                                : true
                                            }
                                            target="_blank">
                                              ...
                                              <span
                                                ...
                                                onClick={() =>
                                                  deleteFileFromList(
                                                    "publicationFileName",
                                                    cohort.publicationFileName[0].filename,
                                                    cohort.publicationFileName[0].fileId,
                                                    cohortID,
                                                  )
                                                }>
                                                x
                      ...
                      <tr>
                        ...
                        <td>
                          <Row className="w-100" style={{ paddingRight: "0" }}>
                            ...
                            <Col
                              style={{ lineHeight: "2em", paddingLeft: "0" }}
                              md={!isReadOnly ? "12" : "11"}
                              xl={!isReadOnly ? "7" : "11"}>
                              {cohort.questionnaire_url.length === 0 && (
                                <span style={{ padding: "0" }}>No URL(s) entered</span>
                              )}
                              {cohort.questionnaire_url.length > 0 && (
                              ...
                              {cohort.questionnaireFileName.length > 0 && (
                                <span>
                                  <a
                                    href={"../../../api/download/" + cohort.questionnaireFileName[0].filename}
                                    download={
                                      cohort.questionnaireFileName[0].filename.split(".").pop() === "pdf" ? false : true
                                    }
                                    target="_blank">
                                    {cohort.questionnaireFileName[0].filename}
                                  </a>
                            ...
                            <Col sm="9" className="pl-0">
                              ...
                              {cohort.mainFileName.length > 0 && (
                                <span>
                                  <a
                                    href={"../../../api/download/" + cohort.mainFileName[0].filename}
                                    download={cohort.mainFileName[0].filename.split(".").pop() === "pdf" ? false : true}
                                    target="_blank">
                                    {cohort.mainFileName[0].filename}
                                  </a>
                                  {!isReadOnly && (
                              ...
                              {cohort.publicationFileName.length > 0 && (
                                <span>
                                  <a
                                    href={"../../../api/download/" + cohort.publicationFileName[0].filename}
                                    download={
                                      cohort.publicationFileName[0].filename.split(".").pop() === "pdf" ? false : true
                                    }
                                    target="_blank">
                                    {cohort.publicationFileName[0].filename}
                                  </a>
