...
export default function CohortActivity() {
  ...
  const toggleHeaderSort = (key) => {
    let newSortOrder = headerSort.hasOwnProperty(key) ? (headerSort[key] === "asc" ? "desc" : "asc") : "asc";
        ...
        <Table bordered condensed>
          <thead>
            <tr>
              {columns.map(({ key, title }) => (
                <th key={`table-header-${key}`} className="c-pointer" onClick={(_) => toggleHeaderSort(key)}>
                  {title}
          ...
          <tbody>
            {activityLog
              ...
              .filter((_, i) => page === -1 || (i >= page * pageSize && i < (page + 1) * pageSize))
              .map((log, rowIndex) => (
        ...
        <div className="d-flex align-items-center justify-content-between">
          ...
          <div className="d-flex">
            <span className="mt-1 mr-2">
              ...
              {page === -1
                ? "All"
                : `${1 + page * pageSize}-${Math.min(activityLog.length, (page + 1) * pageSize)} of ${
                    activityLog.length
                  }`}
            </span>
            <Paging
