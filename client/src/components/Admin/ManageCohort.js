  ...
  render() {
    ...
    let content = list.map((item, index) => {
      ...
      return (
        <tr key={id}>
          <td>
            <Link to={`/admin/activitylog/${item.acronym}`}>{item.name}</Link>
          </td>
          <td>
            <Link to={`/admin/activitylog/${item.acronym}`}>{item.acronym}</Link>
          </td>
          ...
          <td>
            <Link
              onClick={(e) => {
                this.reviewCohort(e, id, item.status);
              }}>
              {["submitted", "in review"].includes(item.status.toLowerCase()) ? "Review" : "View"}
            </Link>
          </td>
        </tr>
      );
    ...
    return (
      <div>
        ...
        <p className="welcome">
          The list below contains all the published and unpublished cohorts currently registered on the CEDCD website.
        </p>
        <p></p>
        <div
          className="d-flex flex-wrap flex-fill justify-content-between mx-4"
          ...
          <div className="d-flex flex-wrap flex-fill justify-content-left">
            <div
              ...
              <input
                ...
                value={this.state.filter.cohortSearch}
                placeholder="Search with key word "
                onChange={(e) => this.handleCohortSearchChange(e)}
              />
              <i
            ...
            <div id="cohortStatus" className="filter-component mr-2" style={{ minWidth: "125px" }}>
              <CohortStatusList
                hasUnknown={true}
                values={this.state.filter.cohortStatus}
                displayMax="0"
            ...
            <div>
              <Link
                ...
                style={{ color: "#23527c", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
