...
const EditUser = ({ ...props }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async function () {
      ...
      if (result.data.info[0] !== undefined) {
        ...
        const allCohorts = result.data.cohortList.map((item, idx) => ({
          value: item.acronym,
          label: item.acronym,
          name: item.name,
        }));
    ...
    const fetchExistingData = async function () {
      ...
      setExistingList(result.data.emailList);
      let alls = result.data.cohortList.map((item, idx) => ({
        value: item.acronym,
        label: item.acronym,
        name: item.name,
      }));
  ...
  const sendEmail = () => {
    ...
    } else html += "<li>No cohorts associated with your account</li>";
  ...
  const validateInput = () => {
    ...
    if (
      (isNull(copy.loginType_error) && currentUser.login_type !== loginType) ||
      (isNull(copy.email_error) && currentUser.email !== userEmail) ||
      isNew
    ) {
  ...
  return (
    <div id="editUserContainer" className="col-md-12">
      ...
      <CenterModal
        ...
        footer={
          <div>
            <Button className="col-lg-2 col-md-6" variant="primary" onClick={() => setModalShow(false)}>
              OK
            </Button>
          </div>
        }
        ...
        <div id="edituser-main" className="col">
          ...
          {nonExistUser ? (
            <div className="col-md-12 col-6">
              <div className="col-md-12 col-6">
                {" "}
                <h4> Non existing user id</h4>{" "}
              </div>
              <div className="bttn-group col-md-4 col-xs-6">
                <input
                  type="button"
                  className="col-md-1 col-xs-6 btn btn-primary"
                  value="Cancel"
                  onClick={goBack}
            ...
            <div id="edituser-col-1" className="col-md-6 col-6">
              <Form>
                  ...
                  <span className="col-md-12 col-12" style={{ paddingLeft: "0" }}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.lastName_error !== "") setErrors({ ...errors, lastName_error: "" });
                      }}
                    />
                  </span>
                </Form.Group>
                  ...
                  <span className="col-md-12 col-12" style={{ paddingLeft: "0" }}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (errors.firstName_error !== "") setErrors({ ...errors, firstName_error: "" });
                      }}
                    />
                  </span>
                </Form.Group>
                ...
                <div className="pl-0 my-3 col-md-12 col-sm-12 col-12" style={{ paddingLeft: "0" }}>
                  <Button variant="primary" value="Save" className="col-lg-2 col-md-6 float-right" onClick={handleSave}>
                    Save
                  </Button>
                  <Button variant="secondary" className="col-lg-2 col-md-6 float-right" onClick={goBack}>
                    Cancel
                  </Button>
                </div>
              </Form>
