...
class ManageUser extends Component {
  ...
  refreshDataList(pageIndex, userNameSearch, userStatus, pageSize, orderByColumn) {
    ...
    if (["name", "action"].includes(orderBy.column)) {
      list = list.sort((a, b) => {
        return orderBy.order === "asc" ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name);
      });
      ...
      list = list.sort((a, b) => {
        ...
        if (aValue == bValue) {
          ...
          return orderBy.order === "asc"
            ? (aValue === "Never" ? new Date("01/01/3000").getTime() : new Date(aValue).getTime()) -
                (bValue === "Never" ? new Date("01/01/3000").getTime() : new Date(bValue).getTime())
            : (bValue === "Never" ? new Date("01/01/2000").getTime() : new Date(bValue).getTime()) -
                (aValue === "Never" ? new Date("01/01/2000").getTime() : new Date(aValue).getTime());
        }
      });
    } else {
      list = list.sort((a, b) => {
        ...
        if (aValue == bValue) {
          ...
          return orderBy.order === "asc" ? aValue.localeCompare(bValue) : bValue.localeCompare(aValue);
        }
      });
    }
  ...
  loadingData = (next) => {
    ...
    fetch("/api/managecohort/adminuserlist", {
      ...
      .then((result) => {
        ...
        let list =
          state.filter.userStatus === "Y" ? allDataList.filter((item) => item.active_status === "Y") : allDataList;
  ...
  render() {
      ...
      return (
        <tr key={id}>
          ...
          <td style={{ textDecoration: "underline" }}>
            <Link to={editUserLink} onClick={this.saveHistory}>
              {editUser}
            </Link>
          </td>
        </tr>
    ...
    return (
      <div className="col-md-12 col-12">
        ...
        <p className="welcome">The list below contains all users registered on the CEDCD website.</p>
        <p></p>
        <div
          className="col-md-12 col-12"
          style={{ verticalAlign: "middle", marginBottom: "-15px", paddingBottom: "0px" }}>
          <div className="col-lg-3 col-md-6 col-sm-5 col-8 pl-0" style={{ paddingBottom: "0px" }}>
            <div className="input-group">
              ...
              <input
                ...
                value={this.state.filter.userNameSearch}
          ...
          <div
            className="col-md-2 col-sm-5 col-4"
            style={{
              paddingLeft: "10px",
          ...
          <div
            className="col-lg-2 col-md-4 col-sm-7 col-6"
            style={{
              paddingLeft: "15px ",
            ...
            <input
              type="checkbox"
              id="userActiveStatus"
              checked={this.state.filter.userStatus === "Y"}
              onChange={(e) => this.userStatusClick(e)}
            />
            <label htmlFor="userActiveStatus"> &nbsp;Show Active Users Only</label>
          </div>
          <div
            className="col-lg-5 col-sm-12 col-12"
            style={{ display: "flex", paddingRight: "0px", float: "right", paddingBottom: "0px", marginBottom: "0px" }}>
            <div
              style={{
                marginLeft: "auto",
                paddingLeft: "3px",
