...
const DataLinkageForm = ({ ...props }) => {
      ...
      .then((result) => {
        if (result.data.info[0] !== undefined) {
          ...
          batch(() => {
            ...
            dispatch(
              allactions.dataLinkageActions.setHaveDataLinkSpecify(data.dlh_linked_to_existing_databases_specify),
            );
            dispatch(allactions.dataLinkageActions.setHaveHarmonization(data.dlh_harmonization_projects));
            dispatch(
              allactions.dataLinkageActions.setHaveHarmonizationSpecify(data.dlh_harmonization_projects_specify),
            );
            ...
            if (data.dlh_procedure_online === 0)
              dispatch(
                allactions.dataLinkageActions.dataFileName(
                  files || { fileId: 0, fileCategory: 5, filename: "", status: 0 },
                ),
              );
            //else dispatch(allactions.dataLinkageActions.dataFileName({fileId: 0, fileCategory: 2, filename: '', status: 0}))
            else if (data.dlh_procedure_online === 1) {
              dispatch(allactions.dataLinkageActions.setDataOnlineURL(website));
            } else {
              dispatch(
                allactions.dataLinkageActions.dataFileName({ fileId: 0, fileCategory: 5, filename: "", status: 0 }),
              );
  ...
  const validateInput = () => {
    ...
    //F.1
    if (!(dataLinkage.haveDataLink in [0, 1])) {
      copy.haveDataLink = radioError;
    } else {
      copy.haveDataLink = "";
    }
    if (dataLinkage.haveDataLink === 1) {
      if (!dataLinkage.haveDataLinkSpecify) copy.haveDataLinkSpecify = "Please specify";
      else {
        if (dataLinkage.haveDataLinkSpecify.length > 500) copy.haveDataLinkSpecify = "Cannot exceed 500 characters";
        else copy.haveDataLinkSpecify = "";
      }
    } else copy.haveDataLinkSpecify = "";

    //F.2
    if (!(dataLinkage.haveHarmonization in [0, 1])) {
      copy.haveHarmonization = radioError;
    } else {
      copy.haveHarmonization = "";
    }
    if (dataLinkage.haveHarmonization === 1) {
      if (!dataLinkage.haveHarmonizationSpecify) copy.haveHarmonizationSpecify = "Please specify";
      else {
        if (dataLinkage.haveHarmonizationSpecify.length > 500)
          copy.haveHarmonizationSpecify = "Cannot exceed 500 characters";
        else copy.haveHarmonizationSpecify = "";
      }
    } else copy.haveHarmonizationSpecify = "";

    //F.3
    if (!(dataLinkage.haveDeposited in [0, 1])) {
      copy.haveDeposited = radioError;
    } else {
      copy.haveDeposited = "";
    }
    ...
    //F.4
    if (!(dataLinkage.dataOnline in [0, 1])) {
      copy.dataOnline = radioError;
    } else {
      copy.dataOnline = "";
    }
    if (dataLinkage.dataOnline === 1) {
      if (!dataLinkage.dataOnlineURL) {
        ...
        if (dataLinkage.dataOnlineURL.length > 200) copy.dataOnlineURL = "Cannot exceed 200 characters";
        else copy.dataOnlineURL = "";
      }
    }

    if (dataLinkage.dataOnline === 0 && (!dataLinkage.dataFileName || dataLinkage.dataFileName.status === 0))
      copy.dataFileName = "Required field";
    else copy.dataFileName = "";
    /*
        ...
        */
    //F.5
    if (!(dataLinkage.createdRepo in [0, 1])) {
      copy.createdRepo = radioError;
    } else {
      copy.createdRepo = "";
    }
    if (dataLinkage.createdRepo === 1) {
      if (!dataLinkage.createdRepoSpecify) copy.createdRepoSpecify = "Please specify";
      else {
        if (dataLinkage.createdRepoSpecify.length > 200) copy.createdRepoSpecify = "Cannot exceed 200 characters";
        else copy.createdRepoSpecify = "";
      }
    } else copy.createdRepoSpecify = "";
          ...
          if (result.data) {
            if (result.data.duplicated_cohort_id && result.data.duplicated_cohort_id != cohortId) {
              ...
              window.history.pushState(
                ...
                window.location.pathname.replace(/\d+$/, result.data.duplicated_cohort_id),
              );
      ...
      <Form>
        <CollapsiblePanelContainer>
          ...
          <CollapsiblePanel
            ...
            <Form.Group as={Row}>
              <Form.Label column sm="12">
                <span style={{ fontWeight: "normal" }}>If yes, please specify:</span>
              </Form.Label>
              <Col sm="12">
                {saved && errors.haveDataLinkSpecify ? (
                  <Reminder
                    message={errors.haveDataLinkSpecify}
                    disabled={!errors.haveDataLinkSpecify}
                    placement="right">
                    <Form.Control
                      type="text"
            ...
            <Form.Group as={Row}>
              ...
              <Col sm="12">
                {saved && errors.haveHarmonizationSpecify ? (
                  <Reminder
                    message={errors.haveHarmonizationSpecify}
                    disabled={!errors.haveHarmonizationSpecify}
                    placement="right">
                    <Form.Control
                      type="text"
                      style={{ border: "1px solid red" }}
                    ...
                    <Form.Check.Input
                      ...
                      onClick={() => {
                        if (!isReadOnly) {
                          ...
                          dispatch(setHasUnsavedChanges(true));
                          clone.dataFileName = "";
                          if (!dataLinkage.dataOnlineURL) clone.dataOnlineURL = "Please specify";
                          if (dataLinkage.dataFileName && dataLinkage.dataFileName.status > 0)
                            dispatch(
                              allactions.dataLinkageActions.dataFileName({ ...dataLinkage.dataFileName, status: 0 }),
                            );
                          setErrors(clone);
                        }
                      }}
...
*/}
              ...
              <Form.Label column sm="12" style={{ fontWeight: "normal" }}>
                If no, please attach data sharing plan (PDF):{" "}
                {saved && errors.dataFileName && (
                  <span className="text-danger ml-3 font-weight-normal">{errors.dataFileName}</span>
                )}
              </Form.Label>
              ...
              <Col sm="12" className="pr-0">
                ...
                <Col sm="9" className="px-0">
                  ...
                  {!MfileLoading && dataLinkage.dataFileName.status < 1 && (
                    <span>{dataLinkage.dataOnline ? "" : "No file chosen"}</span>
                  )}

                  {dataLinkage.dataFileName.status > 0 && (
                    <span>
                      <a
                        href={"../../../api/download/" + dataLinkage.dataFileName.filename}
                        download={dataLinkage.dataFileName.filename.split(".").pop() === "pdf" ? false : true}
                        target="_blank">
                        {dataLinkage.dataFileName.filename}
                      </a>
                    </span>
                  )}
                </Col>
