...
const QuestionnaireLoader = (props) => {
  ...
  return (
    <Questionnaire activeSection={current} handler={(section) => setCurrent(section)} isReadOnly={props.isReadOnly}>
      <Content
        currentSection={current}
        readOnlyOrNot={props.isReadOnly}
        sectionJumper={(section) => setCurrent(section)}
      />
    </Questionnaire>
  );
};
