...
actions[t.renewCohort] = (state, action) => ({...action.value})
...
actions[t.addInvestigator] = (state) => ({...state, investigators: [...state.investigators, {personId: 0, name:'', institution: '', email: ''}]})
...
actions[t.setEnrollStartYear] = (state, action) => ({...state, enrollment_year_start: /^\s*\d*\s*$/.test(action.year) ? action.year : state.enrollment_year_start})
actions[t.setEnrollEndYear] = (state, action) => ({...state, enrollment_year_end: /^\s*\d*\s*$/.test(action.year) ? action.year : state.enrollment_year_end})
...
actions[t.setYearToComplete] = (state, action) => ({...state, enrollment_year_complete: /^\s*-?\d*\s*$/.test(action.year) ? action.year : state.enrollment_year_complete})
...
actions[t.setBaseLineMedianAge] = (state, action) => ({...state, enrollment_age_median: /^\s*\d*\s*$/.test(action.medianAge) ? action.medianAge : state.enrollment_age_median})
